...
def generate_bboxes(use_api, model, image_path):
    ...
    if use_api:
        ...
        return generate_local_bboxes(class_dict, model, image_path, image_path, bboxs)
...
def generate_api_bboxes(class_dict, image, draw, bboxs):
    ...
    apiresults = CLIENT.infer("./outputs/prepocessed.png", model_id="circuit-recognition/2")
    ...
    for component in components:
            ...
            cv2.rectangle(bboxs, (corner2[0], corner1[1]), (corner1[0], corner2[1]), (0, 0, 255), 2)
    image.save("outputs/components_removed.png")
    return components, bboxs

def generate_local_bboxes(class_dict, model, image, draw, bboxs, image_path):
    components = []
...
def classify_ocr_results(result):
    ...
    for text1 in result:
        ...
        if "V" in unit or "v" in unit:
            labels.append({"component": "V", "value": value, "corners": text1[0]})
        elif "A" in unit:
            labels.append({"component": "A", "value": value, "corners": text1[0]})
        elif "H" in unit:
            labels.append({"component": "L", "value": value, "corners": text1[0]})
        elif "F" in unit:
            labels.append({"component": "C", "value": value, "corners": text1[0]})
        else:
            labels.append({"component": "R", "value": value, "corners": text1[0]})

    return labels

def ocr(bboxs, threshold):
    # Create an OCR reader object
    reader = easyocr.Reader(lang_list=['en'], gpu=False)
    # Load the image with components removed
    component_removed_image = Image.open("outputs/components_removed.png").convert("RGB")
    draw2 = ImageDraw.Draw(component_removed_image) 

    # Perform OCR on the image
    ocr_results = reader.readtext(image="outputs/components_removed.png", threshold=threshold)
    ...
    for box in ocr_bounding_boxes:
        ...
        cv2.rectangle(bboxs, (x_min, y_min), (x_max, y_max), (255, 0, 0), 2)
...
def calculate_endpoints(classified_results, components):
    ...
    return endpoints, all_connections
...
def formatting_circuit(components, endpoints, all_connections):
