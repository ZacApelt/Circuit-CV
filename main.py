...
# Load the image
image_path = "./circuits/cir10.png"  # specify file location 
use_api = False 
...
def preprocessing():
    ...
    binary = cv2.bitwise_not(binary)
    cv2.imwrite("./outputs/binary_frame.png", binary)

def generate_api_bboxes():
    apiresults = CLIENT.infer("./outputs/binary_frame.png", model_id="circuit-recognition/2")
...
def ocr_removal():
    ...
    ocr_results = reader.readtext("outputs/components_removed.png")
...
def calculate_endpoints(classified_results):
...
def generating_circuit():
    ...
    print(falstad_input)
    return falstad_input

def launching_falstad(falstad_input):
...
if use_api:
    ...
    generate_local_bboxes()

ocr_results = ocr_removal()
calculate_endpoints(ocr_results)

formatting_circuit()
circuit_out = generating_circuit()
launching_falstad(circuit_out)
